...
export class UserProfileService {
  ...
  async getUserProfile(userId: string): Promise<UserProfileDTO> {
    ...
    if (!profile) return this.createDefaultProfile(userId, user);
  ...
  async updateUserProfile(userId: string, params: UpdateUserProfileParams): Promise<UserProfileDTO> {
    const userIdVo = UserId.fromString(userId);
    const user = await this.userRepository.findById(userIdVo);
    if (!user) throw new UserNotFoundError();
    ...
    if (hasUserFields) {
      ...
      if (params.dateOfBirth !== undefined) user.updateDateOfBirth(params.dateOfBirth ? new Date(params.dateOfBirth) : null);
    ...
    if (!profile) {
      ...
      if (params.defaultPaymentMethodId !== undefined) {
        if (params.defaultPaymentMethodId) {
          await this.validatePaymentMethodBelongsToUser(params.defaultPaymentMethodId, userIdVo);
      ...
      if (params.stylePreferences !== undefined) profile.setStylePreferences(params.stylePreferences);
      if (params.preferredSizes !== undefined) profile.setPreferredSizes(params.preferredSizes);
      await this.userProfileRepository.save(profile);
    }
  ...
  async setDefaultPaymentMethod(userId: string, paymentMethodId: string): Promise<void> {
    ...
    let profile = await this.userProfileRepository.findByUserId(userIdVo);
    if (!profile) {
      profile = UserProfile.create({ userId, defaultPaymentMethodId: paymentMethodId });
      await this.userProfileRepository.save(profile);
    } else {
  ...
  private async validateAddressBelongsToUser(addressId: string, userId: UserId): Promise<void> {
    ...
    if (!address.belongsToUser(userId)) throw new InvalidOperationError('Address does not belong to user');
  }

  private async validatePaymentMethodBelongsToUser(paymentMethodId: string, userId: UserId): Promise<void> {
    const pm = await this.paymentMethodRepository.findById(PaymentMethodId.fromString(paymentMethodId));
    if (!pm) throw new PaymentMethodNotFoundError();
    if (!pm.belongsToUser(userId)) throw new InvalidOperationError('Payment method does not belong to user');
  }

  private mapToDTO(profile: UserProfile, user: User): UserProfileDTO {
    const dto = UserProfile.toDTO(profile);
    return {
      ...dto,
      // Merge user entity fields — these are part of the combined profile view
